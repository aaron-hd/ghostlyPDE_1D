...
#             maximum(abs.(hamiltonianTab[end][1:Nt])),
    ...
    energiesPlot = plot(
        layout=(1), framestyle=:box, dpi = 400, size = (400,350), 
        legend=:topleft,
        ...
        #ylims=ylimSetAuto,
        xlabel=L"$t/L$", 
        ylabel=L"$\textrm{decoupled}\;\;\textrm{energies}$",
        left_margin = 40px,
        right_margin = 0px,
        bottom_margin = 10px,
        top_margin = 10px
...
#     chiField = [sol[:, i][j] - offsetchi for i = 1:Nt, j=2N+1:3N];
    ...
    rangeChi = 1.2#maximum((maximum(abs.(chiField)),2.1))/2;
...
#     rangeChi = minimum((maximum(abs.(chiField[1:div(Nt, 2),:])),3.5));
    ...
    densityplot = plot(
        layout=(1), size = (400,350), framestyle=:box, dpi = 400, 
        guidefont = "Computer Modern", tickfont = "Computer Modern",
        xguidefontsize = 16, yguidefontsize = 16, 
        legendfontsize = 16, titlefontsize = 16, tickfontsize = 16,
