...
include(joinpath(level, "src/evolution_PRL.jl"));
include(joinpath(level, "src/hamiltonian_PRL.jl"));
include(joinpath(level, "src/initial_data_gaussian.jl"));
...
function artisan_evolution_at_resolution(Nx, stableRandomSeed, pModel, pInit)
    ...
    (mphi2, mchi2, lam, c, epsDiss) = pModel;
    ...
    p = (dx, mphi2, mchi2, lam, c, epsDiss, Nx, NboundaryPadding);
...
function main()
    ...
    pModel = (mphi2, mchi2, lam, c, epsDiss);
    pInit = (
    ...
    for res in resTab
        ...
        # unpack parameters
        (dx, mphi2, mchi2, lam, c, epsDiss, Nx, NboundaryPadding) = p
        # append the results
        push!(pTab, p)
        push!(solTab, sol)
