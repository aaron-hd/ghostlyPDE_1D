...
#             maximum(abs.(hamiltonianTab[end][1:Nt])),
    ...
    energiesPlot = plot(
        layout=(1), framestyle=:box, dpi = 300, size = (500,400), 
        legend=:topleft,
...
#             label=latexstring("H_\\mathrm{int}"),
...
#             energiesPlot, 
#             sol.t, hamiltonian-hamPhi-hamChi,
#             label=latexstring("H_\\mathrm{int} - H_\\mathrm{int}^{0}"),
#             linewidth=2, linecolor="orange", linestyle=:dot
#         )
    end
...
#     rangeChi = minimum((maximum(abs.(chiField[1:div(Nt, 2),:])),3.5));
    ...
    cgradClip = cgrad([:red, :red], [0, 1, 1])

    # construct clipped gradients
    cgradphi_colors = get(cgradphi.colors, LinRange(0,1,100))
    cgradchi_colors = get(cgradchi.colors, LinRange(0,1,100))
    cgradClip_colors = get(cgradClip, LinRange(0,1,2))
    cgradphiClipped = cgrad(vcat(cgradClip_colors, cgradphi_colors, cgradClip_colors))
    cgradchiClipped = cgrad(vcat(cgradClip_colors, cgradchi_colors, cgradClip_colors))
