...
function finite_differenced_pde_with_bc!(du, u, p, t)
    ...
    (dx, mphi2, mchi2, lambdaCross, lambdaSelf, sigma, Nx, NboundaryPadding) = p
    ...
    Threads.@threads for n = NboundaryPadding + 1:Nx + NboundaryPadding
        ...
        dy3dt[n] = (
            ...
            # potential terms
            - mchi2 * y2[n] 
            - 3 * sigma * lambdaCross * y2[n]^2 * y0[n]^3
            - 8 * lambdaSelf * y2[n]^7
        )
    end
...
function check_field_too_large(u, t, integrator)
    max_value_allowed = 10^7  # Define your threshold here
