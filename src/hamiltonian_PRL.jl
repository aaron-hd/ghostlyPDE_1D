...
function hamiltonian_density(fields_at_t, p)
    # unpack parameters
    dx, mphi2, mchi2, lambda, c, epsDiss, Nx, NboundaryPadding = p
    ...
    for n = NboundaryPadding + 1:Nx + NboundaryPadding + 1
        ...
        ham[n-NboundaryPadding] = (
            # phi
            + 1//2 * y1[n]^2
            + 1//8 * (y0[n + 1] - y0[n - 1])^2 * dx^(-2)
            # chi
            - 1//2 * y3[n]^2
            - 1//8 * (y2[n + 1] - y2[n - 1])^2 * dx^(-2)
            # masses
            + (mphi2*y0[n]^2 - mchi2*y2[n]^2)/2
            # cross-coupling
            + lambda/sqrt(4*y0[n]^2 + (-1 + y0[n]^2 -y2[n]^2)^2)
        )
    end
    ham
end
